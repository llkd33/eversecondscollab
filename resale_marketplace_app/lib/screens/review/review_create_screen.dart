...
class _ReviewCreateScreenState extends State<ReviewCreateScreen> {
        ...
        children: [
          Container(
            ...
            child: widget.transaction.productImage != null
                ? SafeNetworkImage(
                    imageUrl: widget.transaction.productImage!,
                    fit: BoxFit.cover,
                    borderRadius: BorderRadius.circular(8),
                  )
  ...
  Widget _buildRatingSection() {
    return Container(
      ...
      child: Column(
        children: [
          Text('거래는 어떠셨나요?', style: AppStyles.headingSmall),
  ...
  Future<void> _pickImages() async {
    if (_imageFiles.length >= 5) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('최대 5장까지 선택 가능합니다')));
      return;
    }
  ...
  Future<void> _submitReview() async {
    ...
    if (_contentController.text.trim().isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('후기 내용을 작성해주세요')));
      return;
    }
