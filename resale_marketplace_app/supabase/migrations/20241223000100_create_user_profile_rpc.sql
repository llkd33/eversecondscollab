...
-- 카카오 OAuth 콜백 후 사용자 프로필 생성을 위한 함수
...
BEGIN
  ...
  IF final_phone != '' AND EXISTS (SELECT 1 FROM users WHERE phone = final_phone AND id != user_id) THEN
    -- 전화번호가 중복되면 빈 값으로 설정
    final_phone := '';
  END IF;
  
  -- 이메일 중복 확인 (비어있지 않은 경우만)
  IF final_email != '' AND EXISTS (SELECT 1 FROM users WHERE email = final_email AND id != user_id) THEN
...
EXCEPTION
  WHEN unique_violation THEN
    ...
    SELECT json_build_object(
      'success', false,
      'message', '중복 데이터로 인한 생성 실패: ' || SQLERRM,
      'user_id', user_id,
      'action', 'error'
    ) INTO result;
  ...
  WHEN OTHERS THEN
    ...
    SELECT json_build_object(
      'success', false,
      'message', '사용자 프로필 생성 중 오류: ' || SQLERRM,
      'user_id', user_id,
      'action', 'error'
    ) INTO result;
...
-- 함수 실행 권한 부여 (인증된 사용자만)
GRANT EXECUTE ON FUNCTION create_user_profile_safe TO authenticated;

-- 함수 설명 추가
COMMENT ON FUNCTION create_user_profile_safe IS '카카오 OAuth 콜백 후 안전한 사용자 프로필 생성을 위한 RPC 함수';